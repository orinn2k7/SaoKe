...
pdfPath = mainDir / "pdf" / "vtb_CT1111_17_09_2024.pdf"
csvPath = mainDir / "csv" / "vtb_CT1111_17_09_2024.csv"

# Cấu hình CSV
fieldNames = ['transaction_no', 'transaction_date', 'transaction_details', 'transaction_amount', 'transaction_offset_name']
...
def process_pages(start_page, end_page):
    with pdfplumber.open(pdfPath) as pdfFile:
        for page_number in range(start_page, end_page):
            ...
            if page_number == 0:
                raw_transations = raw_transations[2:]

            # Xử lí từng giao dịch
            for [no, date, details, amount, offset_name, account_number] in raw_transations:
                try:
                    transactions.append({
                        ...
                        'transaction_offset_name': offset_name.strip().replace('\n', ' ')
                    })
                except Exception as e:
                    print(f"Lỗi xử lý ({page_number + 1}/{len(pdfFile.pages)}): {[no, date, details, amount, offset_name]}", flush=True)
                    continue

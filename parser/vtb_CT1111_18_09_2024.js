...
const processPDF = async () => {
    try {
        const result = await extractPDF("./pdf/vtb_CT1111_18_09_2024.pdf");
        ...
        pages.forEach(page => {
            ...
            rows.forEach(row => {
                const transaction = {
                    ...
                    transaction_amount: row[3].replaceAll('.', '').replaceAll('\n', ''),
                    transaction_offset_name: row[4].replaceAll('\n', ''),
                    transaction_account_number: row[5].replaceAll('\n', '')
                };
        ...
        const csvWriter = createObjectCsvWriter({
            path: "./csv/vtb_CT1111_18_09_2024.csv",
            header: [
